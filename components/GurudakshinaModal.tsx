...
const GurudakshinaModal: React.FC<GurudakshinaModalProps> = ({ guru, post, onClose, currentUser }) => {
  ...
  return (
     <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 max-w-sm mx-auto animate-fadeIn w-full relative">
        ...
        <div className="flex flex-col items-center space-y-4">
            ...
            <div className="p-2 border dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-700 h-[196px] w-[196px] flex items-center justify-center">
               <div className="bg-white p-2 rounded-md">
                {qrCodeUrl ? (
                    <img src={qrCodeUrl} alt="UPI QR Code" width="164" height="164" />
                ) : (
                    <p className="text-xs text-gray-500 p-4">{t('dakshinaQRGeneration')}</p>
                )}
               </div>
            </div>
        </div>
